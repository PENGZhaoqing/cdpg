...
from Qnet import Qnetwork, copyTargetQNetwork
...
def main():
    ...
    target1 = Qnetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=1, dir=dir,
                       folder=a1_Qnet_folder)
    target32 = Qnetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=False, batch_size=32, dir=dir,
                        folder=a1_Qnet_folder)
    Qnet = Qnetwork(actions_num=action_num, q_ctx=q_ctx, isTrain=True, batch_size=32, dir=dir,
                    folder=a1_Qnet_folder)
        ...
        while steps_left > 0:
            ...
            while not env.game_over():
                ...
                if total_steps % update_interval == 0 and replay_memory.size > replay_start_size:
                    ...
                    Qnet.model.forward(mx.io.DataBatch([state_batch, actions_batch, y_batch],
                                                       []), is_train=True)
                    Qnet.model.backward()
                    Qnet.model.update()
                    ...
                    if training_steps % 10 == 0:
                        loss1 = 0.5 * nd.square(
                            nd.choose_element_0index(Qnet.model.get_outputs()[0], actions_batch) - y_batch)
