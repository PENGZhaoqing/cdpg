...
def main():
    ...
    # Init networks
    a1_AC = ActorCritic(state_dim=148, act_space=action_num, dir=dir, folder=a1_AC_folder, config=config)
    ...
    for epoch in epoch_range:
        ...
        while steps_left > 0:
            ...
            while not all_done:
                _, step_value1, _, step_policy1 = a1_AC.forward(step_ob1, is_train=False)
                ...
                for i in range(num_envs):
                    if not envs[i].done:
                        next_ob, reward, done = envs[i].ple.act(action_map[step_action1[i]])
                        reward = sum(reward)
                        envs[i].add(step_ob1[i], step_action1[i], step_value1[i][0], reward)
                        envs[i].done = done
                ...
                if t == t_max:
                    ...
                    extra_value1 = a1_AC.forward(step_ob1, is_train=False)[1]
                    ...
                    _, episode_value1, _, episode_policy1 = a1_AC.forward(env_ob1, is_train=True)
            ...
            info_str = "Epoch:%d, Episode:%d, Steps Left:%d/%d/%d, Reward:%f, Collisions:%f, fps:%f, Advs:%f/%d, Value:%f, Policys:%f" \
                       % (
