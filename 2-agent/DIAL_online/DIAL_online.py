...
def main():
    ...
    parser.add_argument('--save-log', type=str, default='basic_signal_4/log')
    ...
    a1_AC_last = ActorCritic(state_dim=74, signal_num=signal_num, act_space=action_num, dir=dir, folder=a1_AC_folder,
                             config=config)
    a2_AC_last = ActorCritic(state_dim=74, signal_num=signal_num, act_space=action_num, dir=dir, folder=a2_AC_folder,
                             config=config)
    ...
    if testing:
        ...
        logging_config(logging, dir, save_log, file_name + '_t_max_' + str(t_max))
    else:
        logging_config(logging, dir, save_log, file_name + '_t_max_' + str(t_max))

    copyTargetQNetwork(a1_AC.model, a1_AC_last.model)
    copyTargetQNetwork(a2_AC.model, a2_AC_last.model)

    logging.info('args=%s' % args)
    logging.info('config=%s' % config.__dict__)
    print_params(logging, a1_AC.model)

    zero_gradient_4 = mx.nd.array(np.zeros((t_max * num_envs, signal_num)), ctx=q_ctx)
    zero_gradient_1 = mx.nd.array(np.zeros((t_max * num_envs, 1)), ctx=q_ctx)
    ...
    for epoch in epoch_range:
        ...
        while steps_left > 0:
            ...
            signal_buffer1 = mx.nd.array(np.zeros((num_envs, signal_num)), ctx=q_ctx)
            signal_buffer2 = mx.nd.array(np.zeros((num_envs, signal_num)), ctx=q_ctx)

            env_ob1 = np.zeros((t_max * num_envs, 74))
            env_signal2 = mx.nd.array(np.zeros((t_max * num_envs, signal_num)), ctx=q_ctx)
            env_ob2 = np.zeros((t_max * num_envs, 74))
            env_signal1 = mx.nd.array(np.zeros((t_max * num_envs, signal_num)), ctx=q_ctx)

            while not all_done:

                _, step_value1, signal1, _, step_policy1 = a1_AC.forward(step_ob1, signal_buffer2, is_train=False)
                _, step_value2, signal2, _, step_policy2 = a2_AC.forward(step_ob2, signal_buffer1, is_train=False)
                ...
                signal_buffer1_np = signal_buffer1.asnumpy()
                signal_buffer2_np = signal_buffer2.asnumpy()
                ...
                for i in range(num_envs):
                    if not envs[i].done:
                        ...
                        envs[i].add(step_ob1[i], step_ob2[i], step_action1[i], step_action2[i],
                                    step_value1[i][0],
                                    step_value2[i][0],
                                    reward[0], reward[1], signal_buffer1_np[i], signal_buffer2_np[i])
                ...
                if t == t_max:
                    ...
                    extra_value1 = a1_AC.forward(step_ob1, signal_buffer2, is_train=False)[1]
                    extra_value2 = a2_AC.forward(step_ob2, signal_buffer1, is_train=False)[1]
                    ...
                    a1_AC_last.forward(env_ob1, env_signal2, is_train=True)
                    a2_AC_last.forward(env_ob2, env_signal1, is_train=True)
                    ...
                    for i in range(num_envs):
                        ...
                        env_signal1.extend(envs[i].signal1)
                        env_signal2.extend(envs[i].signal2)

                    env_signal1 = mx.nd.array(env_signal1, ctx=q_ctx)
                    env_signal2 = mx.nd.array(env_signal2, ctx=q_ctx)

                    _, episode_value1, _, _, episode_policy1 = a1_AC.forward(env_ob1, env_signal2, is_train=True)
                    _, episode_value2, _, _, episode_policy2 = a2_AC.forward(env_ob2, env_signal1, is_train=True)

                    a1_AC.model.backward(out_grads=[neg_advs1, value_grads1, zero_gradient_4],)
                    a2_AC.model.backward(out_grads=[neg_advs2, value_grads2, zero_gradient_4])

                    grads_buffer1 = a1_AC.model._exec_group.execs[0].grad_dict['signal'][:]
                    grads_buffer2 = a2_AC.model._exec_group.execs[0].grad_dict['signal'][:]

                    a1_AC_last.model.backward(out_grads=[zero_gradient_4, zero_gradient_1, grads_buffer2])
                    a2_AC_last.model.backward(out_grads=[zero_gradient_4, zero_gradient_1, grads_buffer1])

                    a1_last_grads_dict = a1_AC_last.model._exec_group.execs[0].grad_dict
                    a1_grads_dict = a1_AC.model._exec_group.execs[0].grad_dict
                    a2_last_grads_dict = a2_AC_last.model._exec_group.execs[0].grad_dict
                    a2_grads_dict = a2_AC.model._exec_group.execs[0].grad_dict
