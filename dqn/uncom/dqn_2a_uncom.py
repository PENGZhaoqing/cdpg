...
def main():
    ...
    parser.add_argument('--a2-Qnet-folder', type=str, default='basic/a2_Qnet')
    ...
    a2_Qnet_folder = args.a2_Qnet_folder
    ...
    replay_memory2 = ReplayMemory(state_dim=(74,),
                                  history_length=history_length,
                                  memory_size=replay_memory_size,
                                  replay_start_size=replay_start_size, state_dtype='float32')
    ...
    if testing:
        ...
        logging_config(logging, dir, save_log, 'dqn_com')
    else:
        logging_config(logging, dir, save_log, 'dqn_com')
            ...
            while not env.game_over():
                if replay_memory1.size >= history_length and replay_memory1.size > replay_start_size:
                    ...
                    if do_exploration:
                        ...
                        q_value2 = a2_Qnet_target.forward(current_state2, is_train=False)[0].asnumpy()
                ...
                replay_memory2.append(next_ob[1], action2, reward[1], terminal_flag)
                ...
                if total_steps % update_interval == 0 and replay_memory1.size > replay_start_size:
                    ...
                    next_Qnet1 = a1_Qnet_target.forward(nextstate_batch1, is_train=False)[0]
                    next_Qnet2 = a2_Qnet_target.forward(nextstate_batch2, is_train=False)[0]
                    ...
                    Qnet1 = a1_Qnet.forward(state_batch1, is_train=True)[0]
                    Qnet2 = a2_Qnet.forward(state_batch2, is_train=True)[0]

                    grads1 = np.zeros(Qnet1.shape)
                    grads1[np.arange(grads1.shape[0]), actions1] = (nd.choose_element_0index(Qnet1,actions_batch1) - y_batch1).asnumpy()
                    grads1 = mx.nd.array(grads1, ctx=q_ctx)

                    grads2 = np.zeros(Qnet2.shape)
                    grads2[np.arange(grads2.shape[0]), actions2] = (nd.choose_element_0index(Qnet2,actions_batch2) - y_batch2).asnumpy()
