...
class QNet():
    def __init__(self, state_dim, signal_num, act_space, dir, folder, config):
        ...
        fc1 = mx.symbol.FullyConnected(data=state, name='fc1', num_hidden=128)
        act1 = mx.symbol.Activation(data=fc1, name='relu1', act_type="relu")
        # fc2 = mx.symbol.FullyConnected(data=act1, name='fc2', num_hidden=64)
        # act2 = mx.symbol.Activation(data=fc2, name='relu2', act_type="relu")
        qvalue = mx.symbol.FullyConnected(data=act1, name='qvalue', num_hidden=act_space)
    ...
    def forward(self, state, is_train):
        ...
        data_batch = mx.io.DataBatch(
            data=[mx.nd.array(state, ctx=self.config.ctx)],
            label=None)
