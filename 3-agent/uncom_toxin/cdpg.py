...
def main():
    ...
    # Init networks
    a1_AC = ActorCritic(state_dim=74, act_space=action_num, dir=dir, folder=a1_AC_folder, config=config)
    a2_AC = ActorCritic(state_dim=74, act_space=action_num, dir=dir, folder=a2_AC_folder, config=config)
    a3_AC = ActorCritic(state_dim=74, act_space=action_num, dir=dir, folder=a3_AC_folder, config=config)
    ...
    for epoch in epoch_range:
        ...
        while steps_left > 0:
            ...
            while not all_done:
                ...
                data_batch3 = mx.io.DataBatch(data=[mx.nd.array(step_ob3, ctx=q_ctx)], label=None)
                a3_AC.model.reshape([('data', (num_envs, 74))])
                a3_AC.model.forward(data_batch3, is_train=False)
                _, step_value3, _, step_policy3 = a3_AC.model.get_outputs()
                ...
                if t == t_max:
                    ...
                    data_batch3 = mx.io.DataBatch(data=[mx.nd.array(step_ob3, ctx=q_ctx)], label=None)
                    a3_AC.model.forward(data_batch3, is_train=False)
                    _, extra_value3, _, _ = a3_AC.model.get_outputs()

                    extra_value1 = extra_value1.asnumpy()
                    extra_value2 = extra_value2.asnumpy()
                    extra_value3 = extra_value3.asnumpy()
                    ...
                    a3_AC.model.reshape([('data', (len(env_ob3), 74))])
                    data_batch = mx.io.DataBatch(data=[mx.nd.array(env_ob3, ctx=q_ctx)], label=None)
                    a3_AC.model.forward(data_batch, is_train=True)
                    _, episode_value3, _, episode_policy3 = a3_AC.model.get_outputs()
