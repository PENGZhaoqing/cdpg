...
class ActorCritic(object):
    def __init__(self, state_dim, act_space,dir, folder, config):
        ...
        net = mx.sym.FullyConnected(
            data=state, name='fc1', num_hidden=400, no_bias=True)
        ...
        policy_fc3 = mx.sym.FullyConnected(
            data=net, name='policy_fc3', num_hidden=act_space, no_bias=True)
        policy3 = mx.sym.SoftmaxActivation(data=policy_fc3, name='policy3')
        policy3 = mx.sym.clip(data=policy3, a_min=1e-5, a_max=1 - 1e-5)
        log_policy3 = mx.sym.log(data=policy3, name='log_policy3')
        out_policy3 = mx.sym.BlockGrad(data=policy3, name='out_policy3')
        ...
        # Negative entropy.
        neg_entropy3 = policy3 * log_policy3
        neg_entropy3 = mx.sym.MakeLoss(
            data=neg_entropy3, grad_scale=config.entropy_wt, name='neg_entropy3')

        # Value network.
        value = mx.sym.FullyConnected(data=net, name='value', num_hidden=1)
        self.sym = mx.sym.Group(
            [log_policy1, log_policy2, log_policy3, value, neg_entropy1, neg_entropy2, neg_entropy3, out_policy1,
             out_policy2, out_policy3])
