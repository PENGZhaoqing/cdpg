...
class ActorCritic(object):
    def __init__(self, state_dim, signal_num, act_space, dir, folder, config):
        ...
        signal1 = mx.symbol.Variable("signal1")
        signal2 = mx.symbol.Variable("signal2")
        net = mx.symbol.Concat(state, signal1, name="concat")
        net = mx.symbol.Concat(net, signal2, name="concat")
        ...
        self.model = mx.mod.Module(self.sym, data_names=('state', 'signal1', 'signal2'),
                                   label_names=None)

        self.paralell_num = config.num_envs * config.t_max
        self.model.bind(
            data_shapes=[('state', (self.paralell_num, state_dim)), ('signal1', (self.paralell_num, signal_num)),
                         ('signal2', (self.paralell_num, signal_num))],
    ...
    def forward(self, state, signal, is_train):
        self.model.reshape(
            [('state', (len(state), self.state_dim)), ('signal1', signal[0].shape), ('signal2', signal[1].shape)])
        data_batch = mx.io.DataBatch(
            data=[mx.nd.array(state, ctx=self.config.ctx), signal[0], signal[1]], label=None)
